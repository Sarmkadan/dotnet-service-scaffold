...
{
    /// <summary>
    /// Provides structural validation helpers for <see cref="ProblemDetailsFactoryTests"/> instances,
    /// checking that every declared test method satisfies the constraints required for discovery and execution.
    /// </summary>
    public static class ProblemDetailsFactoryTestsValidation
    {
        /// <summary>
        ...
        public static IReadOnlyList<string> Validate(this ProblemDetailsFactoryTests value)
        {
            ...
            for (Type? type = value.GetType();
                 type is not null && typeof(ProblemDetailsFactoryTests).IsAssignableFrom(type);
                 type = type.BaseType)
            {
        ...
        /// <exception cref="ArgumentNullException"><paramref name="value"/> is <see langword="null"/>.</exception>
        public static bool IsValid(this ProblemDetailsFactoryTests value)
        {
            ArgumentNullException.ThrowIfNull(value);
        ...
        public static void EnsureValid(this ProblemDetailsFactoryTests value)
        {
            ArgumentNullException.ThrowIfNull(value);
            ...
            throw new ArgumentException(
                FormattableString.Invariant($"ProblemDetailsFactoryTests instance is invalid ({problems.Count} problem(s)):{Environment.NewLine}{details}"),
                nameof(value));
        }
